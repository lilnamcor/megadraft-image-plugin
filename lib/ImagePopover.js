...
var ICONS = [{ id: 'small', icon: _svg.smallIcon, size: '25%' }, { id: 'medium', icon: _svg.mediumIcon, size: '50%' }, { id: 'large', icon: _svg.largeIcon, size: '100%' }];
...
var ImageComponent = function (_Component) {
  ...
  _createClass(ImageComponent, [{
    key: 'setSize',
    value: function setSize(size, id) {
    ...
    value: function render() {
      ...
      var icons = ICONS.map(function (icon, index) {
        var fill = _this2.state.active === icon.id ? '#48e79a' : '#fff';
        var style = { fill: fill };
        return _react2.default.createElement(
          'div',
          { onClick: function onClick() {
              return _this2.setSize(icon.size, icon.id);
